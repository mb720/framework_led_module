...
def main():
    ...
    # Argparse requires the --device and brightness-matrix arguments to be provided in the command line
    devices = args.device
...
def matrix_from_string(matrix_as_string):
    ...
    return [[int(value) for value in row.split(',')] for row in matrix_as_string[1:-1].split('],[')]
...
def matrix_to_bytes_for_black_white_draw_cmd(matrix):
  # For the LED module's draw command (0x06) we send 39 bytes to the module. The reason is that with a 34x9 matrix, we need 34*9 = 306 bits to represent the individual LED's state (on or off). 306 bits fit into 39 bytes: 306/8 = 38.25.
  ...
  for row in range(NR_OF_ROWS):
      for col in range(NR_OF_COLUMNS):
          ...
          # print(f"led_index: {led_index}, row: {row}, col: {col}")
          if matrix[row][col]:
              byte_index = int(led_index / 8)
              bit_index = led_index % 8
              # print(f"Turning on bit {bit_index} of byte {byte_index}")
...
def send_command_raw(dev, command, with_response=False):
    import serial
    """Sends a command to the device. Opens a new serial connection every time"""
    try:
...
def get_commandline_args():
    ...
    parser.add_argument("--brightness-matrix", required=True, help="brightness matrix as string. The string specifies a matrix with 9 columns and 34 rows where each row is delimited by brackets, and separated by commas. Three rows for example: \"[100,33,100,33,100,33,100,33,100],[100,32,100,32,100,32,100,32,100],[100,31,100,31,100,31,100,31,100], ...\". The first row in the matrix is the top row on the LED display.")

    parser.add_argument('--device', action='extend', nargs=1, required=True, help='LED device to use. Can be specified multiple times. Example: --device /dev/ttyACM0 --device /dev/ttyACM1')
